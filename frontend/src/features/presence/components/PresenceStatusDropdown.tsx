...
export const PresenceStatusDropdown: React.FC<PresenceStatusDropdownProps> = ({ value, onChange, disabled = false }) => {
  return (
    <label className="flex items-center gap-2 text-xs text-slate-600" htmlFor="presence-status-select">
      Status
      <select
        ...
        disabled={disabled}
        aria-label="Set your presence status"
        onChange={(event) => onChange(event.target.value as PresenceStatus)}
      >
        {OPTIONS.map((option) => (
